...
mcp.tool("book_appointment", { title: z.string(), dateTime: z.string(), attendeeEmail: z.string() }, async ({ title, dateTime, attendeeEmail }) => {
  try {
    ...
    const conflicts = checkRes.data.items || [];
    const isBusy = conflicts.some((event: any) => {
      const eventStart = new Date(event.start.dateTime || event.start.date);
      const eventEnd = new Date(event.end.dateTime || event.end.date);
      ...
      if (!event.start.dateTime) {
        const eventDateStr = eventStart.toISOString().slice(0, 10);
        const candidateDateStr = start.toISOString().slice(0, 10);
        return eventDateStr === candidateDateStr;
      }
    ...
    if (isBusy) {
      return { content: [{ type: "text", text: `Failed. The slot ${start.toLocaleTimeString("en-US", { timeZone: TIME_ZONE })} is already busy.` }] };
    }
    // -------------------------------
